...
export class CouponsService {
  ...
  async validate(code: string, userId: string, subtotal: number, storeId: string) {
    ...
    if (minOrderValue !== null && subtotal < minOrderValue) {
      throw new BadRequestException(`Pedido mínimo de R$ ${minOrderValue.toFixed(2)} para usar este cupom`)
    }
    ...
    const discountPercent = coupon.discountPercent ? Number(coupon.discountPercent) : null
    const discountFixed   = coupon.discountFixed   ? Number(coupon.discountFixed)   : null
